...
#SBATCH --output=log/step2/step2-%j.out
#SBATCH --error=log/step2/step2-%j.out
...
model="MODEL FROM previous step" 
            ...
            for loss in ${loss_list[@]}
                ...
                for data in ${dataset[@]}
                    ...
                    srun torchrun \
                        ...
                        step2_finetuning_training_multiple_negative.py \
                        --model $model \
                        --output "e5_only/${model_name}-step2-${loss}-lr${lr}-temp${temp}-warmup${warmup}-Data${data}-cls${cls}-${optimizer}-${lr_type}-HERO" \
                        --max_seq_length 512 \
                        --batch_size 20 \
                        --learning_rate $lr \
                        --cls $cls \
