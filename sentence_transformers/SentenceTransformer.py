...
"""
    ...
    @property
    def config(self):
        """Return the config from the first module (transformer model)
        
        This property enables compatibility with DeepSpeed and other libraries that expect
        a .config attribute on the model for accessing model configuration parameters
        like hidden_size, vocab_size, etc.
        """
        try:
            first_module = self._first_module()
            # Check if it's a Transformer module with auto_model.config
            if hasattr(first_module, 'auto_model') and hasattr(first_module.auto_model, 'config'):
                return first_module.auto_model.config
            # Check if the first module itself has a config
            elif hasattr(first_module, 'config'):
                return first_module.config
            else:
                raise AttributeError(f"No config found in first module: {type(first_module)}")
        except (StopIteration, AttributeError) as e:
            raise AttributeError(f"SentenceTransformer has no modules with config attribute: {e}")
