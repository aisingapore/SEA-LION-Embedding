...
class MultipleNegativesSymmetricRankingLossReweighting(nn.Module):
    def __init__(
        self,
    ...
    def compute_loss_from_embeddings(self, embeddings: list[Tensor], labels: Tensor) -> Tensor:
        ...
        log_probs = F.log_softmax(forward_scores, dim=1)

        # Positive log-prob for each anchor (accounts for offset when gathered across devices)
        row_idxs = torch.arange(batch_size, device=anchors.device)
        positive_log_probs = log_probs[row_idxs, range_labels]

        # Keep the paper's formulation w_i = (1 - p_i)^gamma, while clamping
        # p_i slightly below 1 for numerical stability near perfect confidence.
        positive_probs = positive_log_probs.exp().clamp(min=0.0, max=1.0 - self.eps)
        weights = (1.0 - positive_probs).clamp(min=self.eps) ** self.gamma

        # Equation (4) in the paper is the expectation of the weighted loss,
        # i.e. a simple mean over samples rather than a normalized weighted mean.
        forward_loss = (-weights * positive_log_probs).mean()
